...
if (!quiet) {
	console.log(`Client ID source: ${flags["client-id"] ? "--client-id flag" : "ENTRA_CLIENT_ID env"}`);
	console.log(`Tenant ID source: ${flags["tenant-id"] ? "--tenant-id flag" : "ENTRA_TENANT_ID env"}`);
}
...
runChecks({ clientId, tenantId }).then((results) => {
