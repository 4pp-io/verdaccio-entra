...
import { EntraTokenPayloadSchema } from "../types/index";
 ...
 * Schema-derived from {@link EntraTokenPayloadSchema} — type and validator stay in sync.
...
function assertEntraPayload(payload: JWTPayload): EntraTokenPayload {
  const result = v.safeParse(EntraTokenPayloadSchema, payload);
  if (!result.success) {
    const issue = result.issues[0];
    const path = issue.path?.map((p) => p.key).join(".") ?? "unknown";
    throw new Error(`Unexpected token shape: claim "${path}" — ${issue.message}`);
  }
  ...
  if (logger) {
    ...
    if (overrides.length > 0) {
      logger.info(
        ...
        "entra: config fields overridden by env vars: @{overrides}",
      );
    }
  }
 ...
 */
export default class EntraPlugin
  extends Plugin<EntraConfig>
  implements pluginUtils.Auth<EntraConfig>
{
    ...
    this._logger.info(
      {
        ...
        jwks: jwksUri(resolved.tenantId, resolved.authority),
      },
      "entra: plugin loaded — issuer=@{issuer} audience=@{audience}",
    );
    debug(
      "plugin loaded — issuer: %s, audience: %s, jwks: %s",
      this._issuer,
      this._audience,
      jwksUri(resolved.tenantId, resolved.authority),
    );
  }
  ...
  public authenticate(user: string, password: string, cb: pluginUtils.AuthCallback): void {
    ...
    (async () => {
      ...
      if (!upn) {
        this._logger.warn(
          { user },
          "entra: auth rejected — token has no identity claim (preferred_username/upn/email)",
        );
        cb(null, false);
        return;
      }
      ...
      this._logger.info(
        { user: upn, groups: groups.length },
        "entra: auth ok user=@{user} groups=@{groups}",
      );
  ...
  private _extractGroups(payload: EntraTokenPayload): string[] {
    ...
    const hasLegacyGroupOverage = payload["hasgroups"] === true && payload.groups === undefined;
