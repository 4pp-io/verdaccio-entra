...
export function detectSwappedIds(token: string, config: EntraConfig): string | undefined {
  try {
    ...
    if (tid && tid.toLowerCase() === clientId.toLowerCase()) {
      return (
        `It looks like clientId and tenantId may be swapped — the token's tid claim (${tid}) matches your configured clientId. ` +
    ...
    if (
      ...
      claims.aud.toLowerCase().includes(tenantId.toLowerCase())
    ) {
      return (
        `It looks like clientId and tenantId may be swapped — the token's audience (${claims.aud}) contains your configured tenantId. ` +
        "Check ENTRA_CLIENT_ID and ENTRA_TENANT_ID."
      );
    }
  } catch {
...
export function enrichJoseError(err: unknown, token: string, config: EntraConfig): string {
  ...
  if (err instanceof joseErrors.JWTClaimValidationFailed) {
    if (err.claim === "aud") {
      return (
        `Token audience mismatch — expected ${AUDIENCE_PREFIX}${config.clientId}. ` +
        (hint ?? "Ensure the MSAL scope matches the Verdaccio app registration.")
      );
    }
    if (err.claim === "iss") {
      return (
        `Token issuer mismatch — expected tenant ${config.tenantId}. ` +
